     ...
     x-data="{ 
        ...
        init() {
            axios.get('/api/v1/customer/bookings').then(res => {
                ...
                this.bookings = [
                    { id: 101, service: { name: 'HVAC Regular Maintenance' }, technician: { name: 'John Mitchell' }, is_emergency: false, property_type: 'Home', scheduled_date: '2026-05-20', time_slot: 'Morning', status: 'pending', address: '123 Pine St', city: 'Los Angeles' }
                ];
            });
        }
     }"
        ...
        <div class="grid gap-8 lg:grid-cols-3">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('booking.create') }}" class="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all shadow-md cursor-pointer">
                        + New Booking
                    </a>
                </div>
                ...
                <div class="space-y-4">
                    <template x-for="b in bookings" :key="b.id">
                        <div class="glass-card rounded-xl p-6 border border-white/5 bg-[#0f1422]/40">
                            ...
                            <div class="grid gap-4 sm:grid-cols-3 text-xs mb-6 text-slate-400">
                                <div>
                                    <span class="uppercase tracking-wider block font-bold">Technician</span>
                                    <span class="text-slate-200 mt-1 block font-semibold" x-text="b.technician?.name || 'Assigning...'"></span>
                                </div>
                                <div>
                                    <span class="uppercase tracking-wider block font-bold">Schedule</span>
                                    <span class="text-slate-200 mt-1 block font-semibold" x-text="(b.scheduled_date || 'Today') + ' (' + b.time_slot + ')'"></span>
                                </div>
                                <div>
                                    <span class="uppercase tracking-wider block font-bold">Address</span>
                                    <span class="text-slate-200 mt-1 block font-semibold line-clamp-1" x-text="b.address + ', ' + b.city"></span>
                                </div>
                            </div>

                            <!-- Actions Row: Add Chat portal connection -->
                            <div class="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
                                <span class="text-[10px] text-slate-400">Emergency Level: <span x-text="b.is_emergency ? 'CRITICAL' : 'Standard'" :class="b.is_emergency ? 'text-red-400 font-bold' : 'text-emerald-400'"></span></span>
                                <a :href="'/chat?bookingId=' + b.id" class="px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-white text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer">
                                    <i data-lucide="message-square" class="h-3.5 w-3.5 text-emerald-400"></i>
                                    Secure Chat Portal
                                </a>
                            </div>
                        </div>

...
export default function CustomerDashboard() {
  ...
  const mapNotifications = (notifs: any[]) => {
    if (!notifs) return []
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (dashboardData.notifications) {
        dashboardData.notifications = mapNotifications(dashboardData.notifications)
      }
      ...
      fallbackData.notifications = mapNotifications(fallbackData.notifications)
  ...
  const handleMarkAsRead = useCallback(async (notifId: number | string) => {
    try {
      ...
      setData(prev => {
        ...
        const unreadCount = updatedNotifs.filter(n => !n.is_read).length
        return {
          ...prev,
          stats: {
            ...prev.stats,
  ...
  const handleMarkAllRead = useCallback(async () => {
    try {
      ...
      setData(prev => {
        const updatedNotifs = prev.notifications.map(n => ({ ...n, is_read: true }))
        return {
          ...prev,
          stats: {
            ...prev.stats,
  ...
  const handleCancelService = async (e: React.FormEvent) => {
    ...
    if (data.active_booking.technician && !cancelReason.trim()) {
      alert("Please provide a reason for cancelling the service.")
      return
    }

    setIsCancelling(true)
    try {
      await api.post(`/bookings/${data.active_booking.id}/cancel`, {
        reason: cancelReason
      })
      ...
      alert(error.response?.data?.message || "Failed to cancel service. Please try again.")
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden relative pb-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="relative w-9 h-9 rounded-lg hover:bg-secondary/50 flex items-center justify-center transition-colors"
              >
                <Bell className="w-4.5 h-4.5 text-muted-foreground" />
                {data.stats.unread_notifications > 0 && (
                  <span className="absolute -top-1 -right-1 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-blue-500 text-[9px] font-bold text-white ring-2 ring-background animate-pulse z-10 shadow-md">
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    className="absolute right-0 top-12 w-80 rounded-2xl glass-card border border-border/50 p-4 shadow-2xl z-55 mr-[-40px] sm:mr-0"
                  >
                      ...
                      {data.notifications.length > 0 ? (
                        data.notifications.map((notif, idx) => (
                          <div
                            ...
                            className={cn(
                              "p-2.5 rounded-xl border transition-all cursor-pointer relative group",
                              notif.is_read
                                ? "bg-secondary/15 border-border/20 hover:bg-secondary/25"
                          ...
                          >
                            <div className="flex items-start gap-2">
                              <div className={cn(
                                ...
                                notif.is_read ? "bg-muted-foreground/30" : "bg-primary animate-pulse"
                              )} />
                              <div className="flex-1 min-w-0">
                                <p className={cn(
                                  "text-[11px] leading-normal text-foreground/95",
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <div className="border-b border-border/50 px-6 py-4 bg-gradient-to-r from-primary/5 to-transparent flex items-center justify-between">
                ...
                {data.active_booking && (
                  <div className="flex items-center gap-3">
                    {["pending", "assigned", "accepted"].includes(data.active_booking.status) && (
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => {
                        ...
                        className="h-7 text-[10px] font-bold px-3 py-0.5 rounded-full bg-destructive/10 text-destructive hover:bg-destructive hover:text-white border border-destructive/20 transition-all"
                      >
                        Cancel Service
                      </Button>
                    )}
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-4 border-t md:border-t-0 border-border/30 pt-3 md:pt-0">
                        ...
                        <div className="shrink-0">
                          {booking.review ? (
                            ...
                            <span className="text-[11px] font-semibold text-destructive bg-destructive/10 px-2.5 py-1 rounded-lg border border-destructive/20">
                              Service Cancelled
                            </span>
                          ) : (
                ...
                {data.notifications.length > 0 ? (
                  data.notifications.map((notif, idx) => (
                    <div
                      ...
                      className={cn(
                        "p-3 rounded-xl flex items-start gap-2.5 transition-all border cursor-pointer",
                        notif.is_read 
                          ? "bg-secondary/15 border-transparent hover:bg-secondary/25" 
                          : "bg-primary/5 border-primary/10 hover:border-primary/20 hover:bg-primary/10 shadow-sm"
                      )}
                    >
                      <div className={cn("mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full", notif.is_read ? "bg-muted-foreground/35" : "bg-primary animate-pulse")} />
                      <div className="flex-1 min-w-0">
                        <p className={cn(
      ...
      <AnimatePresence>
        {cancelModalOpen && data.active_booking && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md overflow-hidden rounded-3xl border border-border/50 bg-card p-6 shadow-2xl relative"
            >
              <div className="absolute top-4 right-4">
                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-secondary" onClick={() => setCancelModalOpen(false)}>
              ...
              <div className="flex flex-col items-center text-center space-y-4 mb-6">
                ...
                <h2 className="text-xl font-bold tracking-tight text-foreground">Cancel Service Request</h2>
                <p className="text-xs text-muted-foreground">
                  Are you sure you want to cancel this service request?
                  {data.active_booking.technician ? " A technician has already been assigned and will be notified." : ""}
                </p>
              </div>

              <form onSubmit={handleCancelService} className="space-y-4">
                {data.active_booking.technician ? (
                  <div className="space-y-2 text-left">
                    ...
                    <textarea
                      ...
                      placeholder="Please tell us why you are cancelling this service... (Required)"
                      className="w-full rounded-xl border border-border/50 bg-background px-4 py-3 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary min-h-[100px] resize-none"
                      required
                    />
                  </div>
                ) : (
                ...
                <div className="flex gap-3 pt-4 border-t border-border/50">
                  <Button type="button" variant="outline" className="flex-1 rounded-xl h-11" onClick={() => setCancelModalOpen(false)} disabled={isCancelling}>
                    Keep Service
                  </Button>
                  <Button type="submit" variant="destructive" className="flex-1 rounded-xl h-11" disabled={isCancelling}>
